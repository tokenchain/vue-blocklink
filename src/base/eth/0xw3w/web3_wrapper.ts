...
export class Web3Wrapper {
    ...
    public async getBlockIfExistsAsync(
        ...
        const blockWithoutTransactionDataWithHexValuesOrNull = await this.sendRawPayloadAsync<BlockWithoutTransactionDataRPC>({
            method,
    ...
    public async getLogsAsync(filter: FilterObject): Promise<LogEntry[]> {
        ...
        if (_.isNumber(fromBlock)) {// @ts-ignore
            fromBlock = utils.numberToHex(fromBlock);
        }
        let toBlock = filter.toBlock;
        if (_.isNumber(toBlock)) {// @ts-ignore
