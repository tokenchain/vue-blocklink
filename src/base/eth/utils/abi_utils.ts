...
function isAbiDataEqual(name_p: ParamName, type: string, x: any, y: any): boolean {
    ...
    if (_.startsWith(type, 'tuple(')) {
        ...
        for (let i = 0; i < types.length; i++) {
            ...
            //
            // @ts-ignore
            const nestedName = _.isString(name_p.names[i]) ? (name_p.names[i] as string) : ((name_p.names[i] as NestedParamName).name_p as string);
            // @ts-ignore
...
function formatABIDataItem(abi: DataItem, value: any, formatter: (type: string, value: any) => any): any {
